...
import NaverImagePrint.GalleryAdapter;
import NaverImagePrint.NaverImage;
import NaverImagePrint.NaverImageItem;
import NaverImagePrint.RestAPI;
import NaverImagePrint.RestListenner;
...
public class SearchActivity extends AppCompatActivity {
    ...
    private AsyncTask<Void, Void, String> mTask=null, rTask=null;
    private Handler mHandler;
    private int timeTask = 0;
    private float addtest1 = 0, addtest2 = 0, acttest1 = 0, acttest2 = 0, sumtest = 0;
    private String rating1, rating2, rating3;
    ...
    long totalImages = 0;
    int final_check = 0, temp_check = 0, handler_count = 0, delaytime = 1000;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mHandler = new Handler() {
            public void handleMessage(Message msg) {
                if(rTask == null) {
                    rTask = new RatingTask();
                    rTask.execute();
                    mHandler.sendEmptyMessage(100);
                }
                else if(rTask.getStatus() != AsyncTask.Status.FINISHED) {
                ...
                if(mTask == null) {
                    ...
                    // Toast.makeText(getApplicationContext(), mTask.getStatus().toString(), Toast.LENGTH_SHORT).show();
                    readTextView.setText("영화 리뷰 검색 수 (좋아요 수 정렬 100개) : " + Math.round(sumtest) + "개");

                    if(rating1 != null && temp_check == 0){
                        button1_1.setText("네티즌 : " + rating3 + " 점");
                        button1_2.setText("평론가 : " + rating2 + " 점");
                        button1_3.setText("관람객 : " + rating1 + " 점");

                        float temp_rating = Float.valueOf(rating1);

                        if (temp_rating >= 8.0) {
                            button1.setBackgroundColor(getResources().getColor(R.color.colorP1));
                        }
                        else if (temp_rating >= 5.0) {
                    ...
                    if(acttest1 > 0 || acttest2 > 0) {
                        double temp_act = Math.round((float)acttest1 / (float)addtest1 * 1000) / 100.0;
                        String temp_str = String.format("%.2f", temp_act);

                        button2_1.setText("긍정 리뷰 : " + Math.round(acttest1) + "개");
                        button2_2.setText("부정 리뷰 : " + Math.round(acttest2) + "개");
                        button2_3.setText("평점 : " + temp_str + " 점");

                        if (Math.round(acttest1 / addtest1 * 100) >= 80) {
                            button2.setBackgroundColor(getResources().getColor(R.color.colorP1));
                        }
                        else if (Math.round(acttest1 / addtest1 * 100) >= 50) {
                    ...
                    if(!nametest.equals("미판정")) {
                        ...
                        if(final_check == 0) {
                            builder.setContentTitle("영화 평점 리뷰 분석기") // required
    ...
    class BackgroundTask extends AsyncTask<Void, Void, String>
    {
        ...
        protected void onPreExecute() {
            target = "http://davichiar1.cafe24.com/MV_NoticeList.php";
            noticedList.clear();
        }
    ...
    class RatingTask extends AsyncTask<Void, Void, String>
    {
        String target;

        @Override
        protected void onPreExecute() {
            target = "http://davichiar1.cafe24.com/MV_RatingList.php";
        }
        ...
        public void onPostExecute(String result) {
            try {
                ...
                rating1 = object.getString("RATING1");
                rating2 = object.getString("RATING2");
                rating3 = object.getString("RATING3");
            }
            catch (Exception e) {
                e.printStackTrace();
            }
